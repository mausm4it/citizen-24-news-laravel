...
<head>
    ...
    <link rel="stylesheet"
        href="https://fonts.googleapis.com/css2?family={{data_get(activeTheme(), 'options.fonts')}}:wght@400;500;600;700&display=swap">
    ...
    :root {
        --primary-color: {
                {
                data_get(activeTheme(), 'options.primary_color')
            }
        }
        ...
        --primary-font: {
                {
                \Config: :get('site.fonts.'.data_get(activeTheme(), 'options.fonts').'')
            }
        }
...
<body class="{{defaultModeCheck()}}">
    ...
    @isset($post)
    @if(!blank(\Request::route()))
    @if(settingHelper('adthis_option')==1 and settingHelper('addthis_public_id')!=null and \Request::route()->getName()
    == "article.detail")
